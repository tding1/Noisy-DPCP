...
int main(int argc, const char** argv) {
    ...
    const double r_limit     = cmdInput[3];//0.7;
    const int num_seg        = (int)(cmdInput[4]);//200;
    const double sigma       = cmdInput[5];//0.1;
    const vec r              = linspace<vec>(0, r_limit, num_seg);
    ...
    vec etaO             = zeros<vec>(num_seg);

    for (int i = 0; i < num_seg; i++) {
        cout << "iter = " << i+1 << ", outlier ratio = " << r[i] << endl;
        int M = ceil(r[i] * N / (1 - r[i]));
        ...
        input_PSGD in;
        output_PSGD out;
        mat noise = 1/sqrt(D)*sigma * randn<mat>(D, N);
